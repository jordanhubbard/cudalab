...
__device__ float note_hz(int midi) {
  return 440.0f * exp2f((midi - 69) / 12.0f);
}
__device__ float tone(float phase, float brightness) {
  return sinf(phase) + sinf(phase * 2.01f) * .32f * brightness + sinf(phase * 3.99f) * .13f * brightness +
         sinf(phase * 7.02f) * .045f;
}
...
CUDALAB_AUDIO {
  ...
  if (i >= sample_count)
    return;
  unsigned long long n = sample_offset + i;
  float t = n / (float)sample_rate;
  int progression[4] = {45, 41, 48, 43};
  int root = progression[((int)(t / 4)) & 3];
  float local = fmodf(t, 4.0f), mix = 0, left = 0, right = 0;
  int intervals[6] = {0, 7, 12, 16, 19, 24};
#pragma unroll
  for (int v = 0; v < 6; v++) {
    float start = floorf(t * 2 - v * .37f) * .5f + v * .37f, age = t - start;
    if (age < 0)
      age += .5f;
    float env = expf(-age * (2.2f + v * .18f));
    float freq = note_hz(root + intervals[v] + (params.mouse_y > .66f ? 2 : 0));
    float phase = 6.283185f * freq * t;
    float voice = tone(phase, .8f) * env * (.12f / (1 + v * .18f));
    float pan = .5f + .42f * sinf(v * 2.1f + local * .22f + params.mouse_x * 2);
    left += voice * sqrtf(1 - pan);
    right += voice * sqrtf(pan);
    mix += voice;
  }
...
__device__ float sd_segment(float2 p, float2 a, float2 b) {
  float2 pa = make_float2(p.x - a.x, p.y - a.y), ba = make_float2(b.x - a.x, b.y - a.y);
...
CUDALAB_RENDER {
  ...
  for (int tree = 0; tree < 7; tree++) {
    ...
    for (int branch = 0; branch < 5; branch++) {
      ...
      color.y += twig * .10f;
      color.z += twig * .055f;
      float fruit = hypotf(uv.x - b.x, uv.y - b.y);
      float glow = expf(-fruit * (32 - 8 * pulse));
      float hue = .5f + .5f * sinf(tree * 3.1f + branch * 2.4f + params.time * .3f);
      color.x += glow * (.8f + .9f * pulse);
      color.y += glow * (.12f + .35f * hue);
