...
__device__ float scene(float3 p,float& trap){
  float3 z=p; float dr=1, r=0; trap=10;
  #pragma unroll
  for(int i=0;i<12;i++){
    r=sqrtf(dot3(z,z)); trap=fminf(trap,fabsf(z.x*z.y*z.z)); if(r>2.3f)break;
    float theta=acosf(z.z/(r+1e-8f)),phi=atan2f(z.y,z.x),zr=powf(r,8.0f);
    dr=powf(r,7.0f)*8.0f*dr+1.0f; theta*=8;phi*=8;
...
__device__ float softshadow(float3 ro,float3 rd){
  float shade=1,t=.03f;
  for(int i=0;i<40;i++){float trap;float h=scene(add(ro,mul(rd,t)),trap);shade=fminf(shade,18*h/t);t+=fminf(.12f,fmaxf(.01f,h));if(h<.0005f||t>6)break;}
...
CUDALAB_KERNEL {
  ...
  float2 q=make_float2((2.0f*x-params.width)/params.height,(params.height-2.0f*y)/params.height);
  ...
  float travel=0,trap=0,lastTrap=0; bool hit=false;
  int iterations=72+params.quality*24;
  for(int i=0;i<iterations;i++){
    float distance=scene(add(ro,mul(rd,travel)),trap);lastTrap=trap;
    if(distance<.0008f){hit=true;break;} travel+=distance*.72f;if(travel>8)break;
  }
  ...
  if(hit){
    ...
    float stars=powf(cudalab_saturate((sinf(rd.x*831+rd.y*421+rd.z*313)*43758.5f-floorf(sinf(rd.x*831+rd.y*421+rd.z*313)*43758.5f)-.994f)/.006f),9);
    color=add(color,mul(make_float3(.7f,.82f,1),stars));
  }
  float fog=1-expf(-.018f*travel*travel);color=add(mul(color,1-fog),mul(make_float3(.015f,.025f,.07f),fog));
