...
__device__ V mul(V a, float s) { return v(a.x*s, a.y*s, a.z*s); }
__device__ float dot(V a, V b) { return a.x*b.x+a.y*b.y+a.z*b.z; }
__device__ float len(V a) { return sqrtf(dot(a,a)); }
__device__ V norm(V a) { return mul(a, rsqrtf(dot(a,a)+1e-8f)); }
...
__device__ float hash(V p) { float h=sinf(dot(p,v(127.1f,311.7f,74.7f)))*43758.5453f; return h-floorf(h); }
...
CUDALAB_KERNEL {
  ...
  V col=v(0,0,0), p=ro; float trans=1, glow=0, oldY=p.y; bool swallowed=false;
  const int steps=100+params.quality*35;
  for(int i=0;i<steps;i++) {
    const float r=len(p); if(r<.82f){swallowed=true;break;} if(r>24&&i>8)break;
    const float ds=fminf(.19f,fmaxf(.028f,r*.038f));
    rd=norm(sub(rd,mul(p,1.52f/(r*r*r+.08f)*ds))); V np=add(p,mul(rd,ds));
    if((oldY*np.y<=0||fabsf(np.y)<.025f)&&trans>.01f) {
      ...
      if(rr>1.28f&&rr<7.4f) {
        ...
        col=add(col,mul(hot,e*trans*1.9f)); trans*=fmaxf(.12f,1-e*.72f);
      }
    }
    glow+=expf(-fabsf(r-1.05f)*7)*ds*.055f; oldY=np.y; p=np;
  }
  if(!swallowed) {
    const float cell=190; V id=v(floorf(rd.x*cell),floorf(rd.y*cell),floorf(rd.z*cell));
    const float h=hash(id), star=powf(sat((h-.985f)/.015f),7);
    const float neb=powf(sat(.5f+.5f*sinf(rd.x*7+rd.z*5+sinf(rd.y*11))),5);
    col=add(col,mul(add(mul(v(.006f,.009f,.025f),1+neb*2),mul(v(.8f,.82f,1.15f),star*2.8f)),trans));
  }
