...
__device__ float2 sample(const float2* f, int x, int y) {
  x = (x + N) % N; y = (y + N) % N; return f[y * N + x];
}

CUDALAB_RESET {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= CELLS || state_bytes < sizeof(float2) * CELLS * 2ull) return;
  int x = i % N, y = i / N; float nx = (x - N*.5f) / N, ny = (y - N*.5f) / N;
  ...
  auto* fields = static_cast<float2*>(state); fields[i] = value; fields[CELLS+i] = value;
}
...
CUDALAB_SIMULATE {
  ...
  int x=i%N,y=i/N; float2 c=sample(src,x,y);
  float2 lap=make_float2(-c.x,-c.y);
  float2 n=sample(src,x,y-1),s=sample(src,x,y+1),e=sample(src,x+1,y),w=sample(src,x-1,y);
  lap.x += .2f*(n.x+s.x+e.x+w.x); lap.y += .2f*(n.y+s.y+e.y+w.y);
  float2 a=sample(src,x-1,y-1),b=sample(src,x+1,y-1),d=sample(src,x-1,y+1),q=sample(src,x+1,y+1);
  lap.x += .05f*(a.x+b.x+d.x+q.x); lap.y += .05f*(a.y+b.y+d.y+q.y);
  ...
  if(params.mouse_x>0 && params.mouse_y>0) {
    ...
    if(dx*dx+dy*dy<.0012f) v=fminf(1.0f,v+.13f);
  }
...
CUDALAB_RENDER {
  ...
  if(x>=params.width||y>=params.height||!state)return;
  auto* fields=static_cast<float2*>(state);
  const float2* current=fields+((params.frame&1)?0:CELLS);
  int gx=x*N/params.width,gy=y*N/params.height; float2 c=current[gy*N+gx];
  ...
  float3 ink=make_float3(.015f+.08f*bloom,.025f+.36f*bloom+.6f*edge,.055f+.72f*bloom+.2f*sinf(c.y*18));
  float3 gold=make_float3(1.4f,.36f,.035f); ink.x+=gold.x*edge;ink.y+=gold.y*edge;ink.z+=gold.z*edge;
  ink=cudalab_tonemap(ink);
  pixels[y*params.width+x]=make_uchar4(255*powf(cudalab_saturate(ink.x),.4545f),255*powf(cudalab_saturate(ink.y),.4545f),255*powf(cudalab_saturate(ink.z),.4545f),255);
}
